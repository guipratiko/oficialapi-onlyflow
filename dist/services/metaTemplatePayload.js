...
exports.buildMessageTemplatePayload = buildMessageTemplatePayload;
...
function getFooterComponent(components) {
...
function getBodyVariableCount(bodyText) {
...
function validateTemplateCreate(body) {
    ...
    if (!bodyComp)
        throw new Error('O template deve ter um componente de corpo (body).');
    const bodyText = String(bodyComp.text || '').trim();
    if (bodyText.length < META.BODY_MIN)
    ...
    if (numVars > 0) {
        ...
        if (textWithoutVars.length < minChars) {
            throw new Error(`Para ${numVars} variável(is) no corpo, é necessário pelo menos ${minChars} caracteres de texto (excluindo as variáveis). Reduza variáveis ou aumente o texto.`);
        }
        const examples = bodyComp.example?.body_text;
        const flatExamples = Array.isArray(examples)
            ? (Array.isArray(examples[0]) ? examples[0] : examples)
            : [];
        if (flatExamples.length < numVars) {
            throw new Error(`O corpo tem ${numVars} variável(is) ({{1}}, {{2}}...). Informe um texto de amostra para cada uma (ex: João para {{1}}).`);
        }
    }
    ...
    for (let i = 0; i < buttons.length; i++) {
        ...
        if (!t)
            throw new Error(`Texto do botão ${i + 1} é obrigatório.`);
 ...
 */
function buildMessageTemplatePayload(body) {
    const comps = body.components || [];
    ...
    const bodyComp = getBodyComponent(comps);
    if (!bodyComp)
        throw new Error('Componente body é obrigatório.');
    ...
    if (numVars > 0) {
        ...
        if (Array.isArray(raw) && raw.length > 0) {
            if (Array.isArray(raw[0])) {
                values = raw[0].slice(0, numVars);
            }
            else {
                values = raw.slice(0, numVars);
            }
        }
            ...
            const btnType = sub === 'url'
                ...
                : sub === 'phone_number'
                    ...
                    : sub === 'copy_code'
                        ? 'copy_code'
                        : 'quick_reply';
            const item = { type: btnType, text: (b.text || '').trim() || 'Botão' };
            if (btnType === 'url' && b.url)
                item.url = b.url.trim();
            if (btnType === 'phone_number' && b.url)
                item.phone_number = b.url.trim().replace(/\D/g, '').slice(-20);
            if (btnType === 'url' && b.example?.button_url?.length)
                item.example = b.example.button_url;
            buttons.push(item);
        }
